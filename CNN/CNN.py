...
ruscorpora_data.columns = ["word", "value"]

# Use deserialized data
i = 0
for row in ruscorpora_data["word"]:
    ruscorpora_data["word"][i] = row.split("_", 1)[0]
    i += 1
i = 0
...
tokenizer.fit_on_texts(ruscorpora_data["word"])
...
# preparing reviews
reviews_train_prepared = pad_sequences(reviews_to_test.to_numpy(), maxlen=300, padding='post')
reviews_test_prepared = pad_sequences(reviews_to_train.to_numpy(), maxlen=300, padding='post')
...
embedding_matrix = create_embedding_matrix(ruscorpora_data, tokenizer.word_index, 300)
